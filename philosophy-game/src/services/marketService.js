...
export const createMarketRoom = async (hostName, config = {}) => {
  ...
  await set(roomRef, {
    ...
    createdAt: Date.now(),
    hostId,
    config: { numTeams, playersPerTeam, startingXu, auctionTimer, missionTimer },
    teams,
    currentRound: -1,
    players: {
...
export const startAuction = async (roomCode, itemId, startPrice, duration) => {
  const roomRef = ref(database, `rooms/${roomCode}`);
...
export const placeBid = async (roomCode, teamId, amount) => {
  const auctionRef = ref(database, `rooms/${roomCode}/auction`);
  const snap = await get(auctionRef);
  if (!snap.exists()) return { success: false, error: "Không có phiên đấu giá" };

  const auction = snap.val();
  if (!auction.isActive) return { success: false, error: "Phiên đã kết thúc" };

  const now = getServerTime();
  if (now > auction.endTime) return { success: false, error: "Hết thời gian" };

  if (amount <= auction.currentBid) {
    return { success: false, error: "Giá phải cao hơn giá hiện tại" };
  }

  // Check team has enough xu
  const teamSnap = await get(ref(database, `rooms/${roomCode}/teams/${teamId}`));
  if (!teamSnap.exists()) return { success: false, error: "Đội không tồn tại" };
  const teamXu = teamSnap.val().xu || 0;
  if (amount > teamXu) return { success: false, error: "Không đủ Xu" };

  const updates = {};
  updates["currentBid"] = amount;
  updates["currentBidder"] = teamId;
  // Extend timer by 5s if less than 5s remaining
  if (auction.endTime - now < 5000) {
    updates["endTime"] = now + 5000;
  }

  const bidRef = push(ref(database, `rooms/${roomCode}/auction/bids`));
  await set(bidRef, { teamId, amount, timestamp: now });
...
export const endAuction = async (roomCode) => {
  ...
  if (auction.currentBidder) {
    const teamId = auction.currentBidder;
    ...
    updates[`teams/${teamId}/xu`] = Math.max(0, teamXu - price);
